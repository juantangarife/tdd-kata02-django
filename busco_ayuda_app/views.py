...
from .models import Trabajador, Comentario, TiposDeServicio
...
def index(request):
    ...
    return render(request, 'busco_ayuda_app/index.html', context)
...
def editar_perfil(request, idTrabajador):
    ...
    if request.method == 'POST':
        ...
        if form_trabajador.is_valid():
            ...
            messages.success(request, "Su perfil fue actualizado correctamente", extra_tags="alert-success")
            return HttpResponseRedirect('/')
    ...
    return render(request, 'busco_ayuda_app/editar.html', context)
...
def detalle_trabajador(request):
    return render(request, "busco_ayuda_app/detalle.html")
